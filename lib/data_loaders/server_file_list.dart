...
Future<(List<String>, List<String>)> getOfficialFileList(List<String> fileList) async {
  //separate servers
  List<String> tempMasterFiles = fileList.where((element) => element.isNotEmpty && element.trim().substring(element.length - 2, element.length - 1) == 'm').toList();
  List<String> returnMasterFiles = [];
  for (var line in tempMasterFiles) {
    returnMasterFiles.add(line.split('.pat').first);
  }

  List<String> tempPatchFiles = fileList.where((element) => element.isNotEmpty && element.trim().substring(element.length - 2, element.length - 1) == 'p').toList();
